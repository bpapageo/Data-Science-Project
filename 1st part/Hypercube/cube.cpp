...
int main(int argc,char *argv[]){
    ...
    size_t len = 0;
	int k=3,M=10,numoflines=0,i=0,counter,numq,probes=2;
	...
	if((flag1==1)&&(flag2==1)&&(flag3==1)){
    	...
    	///construct cube
		hmap=new hashtable(pow(2.0,k),k,dim,metric,probes);
        ...
        while ((getline(&linee, &len, stream)) != -1) {
			...
			hmap->insert(v[counter-1],metric);
	    ...
	    while ((getline(&linee, &len, stream)) != -1) {
			...
			///LSH Range search
			if(R!=0){
				hmap->Rangesearch(v,metric,R,output);
			}
			...
			///LSH NN search
			time1=clock();
			myvector* res=hmap->search(v,metric);
			time2=clock();
			//statistics 
			double tLSH = (double)(time2-time1)/CLOCKS_PER_SEC;
			sum+=tLSH;
			output<<"Nearest neighbor: "<<res->nameid<<endl<<"distanceCubeLSH: "<<res->dis<<endl<<"distanceTrue: "<<ress->dis<<endl;
			output<<"tCubeLSH: "<<tLSH<<endl<<"tReal: "<<tReal<<endl;
	...
	cout<<"Mean tCubeLSH: "<<sum/numq<<endl;
